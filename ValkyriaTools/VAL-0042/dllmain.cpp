...
/**
    Game: 娘くずし～父と娘の狂縁～(DL版)
    Version: VAL-0042
    https://vndb.org/v21726
*/
...
namespace Hook {
    ...
    const char* wdTitleName = "娘くずし～父と娘の狂縁～ Test";
    ...
    void __stdcall hook_script_read(DWORD esi, HANDLE hFile, DWORD fileInfo, LONG offset) {
        ...
        printf("fileName: %s\n", fileName);
        int32_t* fileSize = (int32_t*)(esi + 0x0C);
        intptr_t* buffer = (intptr_t*)(esi + 0x08);
        ...
        if (GetFileAttributesA(filePath.c_str()) != INVALID_FILE_ATTRIBUTES) {
            ...
            *buffer = (intptr_t)(new char[*fileSize]);
            fread((char*)*buffer, 0x01, *fileSize, fp);
            fclose(fp);
        ...
        __original:
            ...
            *fileSize = *(int32_t*)fileInfo;
            //printf("fileSize: 0x%X\n", (int)*fileSize);
            *buffer = (intptr_t)(new char[*fileSize]);
            SetFilePointer(hFile, offset, 0x00, 0x00);
            ReadFile(hFile, (LPVOID)*buffer, *fileSize, &lpNumberOfBytesRead, 0x00);
        }
        CloseHandle(hFile);
    }
    ...
    void Init() {
        if (BaseAddr = (DWORD)GetModuleHandleA(NULL)) {
            Mem::MemWrite(BaseAddr + 0x9A6E4, &wdTitleName, 0x04); // 修改窗口标题
            uint8_t code[] = {
                0xFF, 0x75, 0xFC,       // push dword ptr ss:[ebp-0x4]  <- offset
            ...
            intptr_t orgAddr = BaseAddr + 0x1964B;
            ...
            Mem::MemWrite(BaseAddr + 0x19767, &code[0x0E], 0x06); // 去除校验判断
        }
